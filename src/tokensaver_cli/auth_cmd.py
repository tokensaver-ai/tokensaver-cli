...
def _api_host(*, force_local: bool = False) -> str:
    ...
    # Local only when asked (--local / TOKENSAVER_MODE=local). A monorepo .env that
    # exports TOKENSAVER_API_URL=http://localhost:8000 must not hijack bare ``login``.
    want_local = force_local or explicit in {"local", "self-host", "selfhost", "self_host"}
    if want_local:
...
def _console_url(*, force_local: bool = False, api_host: str | None = None) -> str:
    ...
    want_local = force_local or explicit in {"local", "self-host", "selfhost", "self_host"}
    host = api_host
    if host is None and want_local:
        host = LOCAL_API_HOST
    mode = "local" if want_local or (host and _is_local_host(host)) else "saas"
    default = LOCAL_CONSOLE_URL if mode == "local" else PROD_CONSOLE_URL
    env_console = (os.environ.get("TOKENSAVER_CONSOLE_URL") or "").rstrip("/")
    if env_console and (want_local or not _is_local_host(env_console)):
        return env_console
    if want_local:
