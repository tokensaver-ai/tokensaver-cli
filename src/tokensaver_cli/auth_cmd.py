...
def _api_host(*, force_local: bool = False) -> str:
    """Resolve API host for auth commands.

    Bare ``tokensaver login`` (no ``--local``) always targets SaaS, even when the
    shell still has ``TOKENSAVER_MODE=local`` or a monorepo ``.env`` pointing at
    localhost — those leftovers were hijacking SaaS login.
    Local host requires an explicit ``--local`` flag.
    """
    import os

    from tokensaver_cli.config import _is_local_host

    env_url = (
        os.environ.get("TOKENSAVER_API_URL") or os.environ.get("TOKENSAVER_API_BASE_URL") or ""
    ).rstrip("/")
    if env_url.endswith("/api/v1"):
        env_url = env_url[: -len("/api/v1")]

    if force_local:
        return env_url or LOCAL_API_HOST

    if env_url and not _is_local_host(env_url):
        return env_url
...
def _console_url(*, force_local: bool = False, api_host: str | None = None) -> str:
    ...
    from tokensaver_cli.config import (
        PROD_CONSOLE_URL,
        _is_local_host,
        resolve_local_console_url,
    )

    if force_local:
        return resolve_local_console_url()

    host = api_host or PROD_API_HOST
    if _is_local_host(host):
        return resolve_local_console_url()

    env_console = (os.environ.get("TOKENSAVER_CONSOLE_URL") or "").rstrip("/")
    if env_console and not _is_local_host(env_console):
        return env_console
    return PROD_CONSOLE_URL
...
def run_login(
    *,
    ...
    mode_env = (os.environ.get("TOKENSAVER_MODE") or "").strip()
    if not force_local and mode_env.lower() in {"local", "self-host", "selfhost", "self_host"}:
        print(
            f"NOTE  TOKENSAVER_MODE={mode_env} is set in this shell but ignored for login "
            "(no --local). Logging into SaaS. Unset it or use: tokensaver login --local",
            file=sys.stderr,
        )
    ...
    try:
        if do_signup:
            ...
            body = {
                ...
                # Backend creates/hashes the API key; OSS never receives or stores the secret.
                "create_key": True,
            }
    ...
    # OSS stores session JWT only (same as console). Backend maps JWT → active API key.
    # Never persist api_key_plain / ts_… secrets from signup or local recovery.
    workspace_id = None
    ...
    print("    Auth:       session JWT (backend maps to your API key — no secret on disk)")
    ...
    print("  tokensaver flows --open   # Flux IA — watch every Claude run live in the control plane")
    ws = workspace_id or (creds.workspace_id if creds else None)
    if ws:
        print(f"  Flux IA → {console}/fr/{ws}/dashboard?tab=flows  (real-time pipeline traces)")
    else:
...
def run_whoami(*, force_local: bool = False) -> int:
    ...
    if token:
        print("  Auth:      session JWT (backend maps to API key)")
    elif api_key:
        import os
