...
from tokensaver_cli.loop_cmd import run_loop
...
from tokensaver_cli.quota_cmd import run_quota
...
Usage:
  ...
  tokensaver quota [--full] [--local]
  tokensaver flows [--open] [--local]
  tokensaver loop start|tick|end|status|precheck|env [options] [--local]
  ...
  tokensaver status [--line|--hook|--session-start|--prompt-nudge]
  tokensaver profiles
...
Auth (no UI required):
  ...
  quota     Quotas + usage (fast, one API call)
  flows     Flux IA URL + recent runs
  loop      ACP-9 business loops (start/tick/end/status) — vibe coding governance
...
Examples:
  ...
  tokensaver loop start --kind goal_based --max-turns 5
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--prompt-nudge", action="store_true", help=argparse.SUPPRESS)
    ...
    parser.add_argument("--full", action="store_true", help="quota: include detailed dimensions")
    parser.add_argument("--export", action="store_true", help="loop: print shell exports")
    parser.add_argument("--json", action="store_true", help="loop status: JSON")
    parser.add_argument("--kind", default=None, help="loop start: loop_kind")
    parser.add_argument("--goal", default=None, help="loop start: goal expression")
    parser.add_argument("--max-turns", type=int, default=None, help="loop start: max turns")
    parser.add_argument("--budget-usd", type=float, default=None, help="loop start: max cost USD")
    parser.add_argument("--loop-id", default=None, help="loop: explicit loop id")
    parser.add_argument("--reason", default=None, help="loop end: stop_reason")
    parser.add_argument("--iteration", type=int, default=None, help="loop tick/precheck: iteration")
    parser.add_argument("--agent-id", default=None, help="loop start: agent_id")
...
def _parse_unknown_route_flags(unknown: list[str]) -> tuple[bool, list[str], dict]:
    ...
    while i < len(unknown):
        ...
        if tok == "--json":
            ...
            i += 1
            continue
        if tok == "--kind" and i + 1 < len(unknown):
            extras["kind"] = unknown[i + 1]
            i += 2
            continue
        if tok.startswith("--kind="):
            extras["kind"] = tok.split("=", 1)[1]
            i += 1
            continue
        if tok == "--goal" and i + 1 < len(unknown):
            extras["goal"] = unknown[i + 1]
            i += 2
            continue
        if tok.startswith("--goal="):
            extras["goal"] = tok.split("=", 1)[1]
            i += 1
            continue
        if tok == "--max-turns" and i + 1 < len(unknown):
            extras["max_turns"] = int(unknown[i + 1])
            i += 2
            continue
        if tok.startswith("--max-turns="):
            extras["max_turns"] = int(tok.split("=", 1)[1])
            i += 1
            continue
        if tok == "--budget-usd" and i + 1 < len(unknown):
            extras["budget_usd"] = float(unknown[i + 1])
            i += 2
            continue
        if tok.startswith("--budget-usd="):
            extras["budget_usd"] = float(tok.split("=", 1)[1])
            i += 1
            continue
        if tok == "--loop-id" and i + 1 < len(unknown):
            extras["loop_id"] = unknown[i + 1]
            i += 2
            continue
        if tok.startswith("--loop-id="):
            extras["loop_id"] = tok.split("=", 1)[1]
            i += 1
            continue
        if tok == "--reason" and i + 1 < len(unknown):
            extras["reason"] = unknown[i + 1]
            i += 2
            continue
        if tok.startswith("--reason="):
            extras["reason"] = tok.split("=", 1)[1]
            i += 1
            continue
        if tok == "--iteration" and i + 1 < len(unknown):
            extras["iteration"] = int(unknown[i + 1])
            i += 2
            continue
        if tok.startswith("--iteration="):
            extras["iteration"] = int(tok.split("=", 1)[1])
            i += 1
            continue
        if tok == "--agent-id" and i + 1 < len(unknown):
            extras["agent_id"] = unknown[i + 1]
            i += 2
            continue
        if tok.startswith("--agent-id="):
            extras["agent_id"] = tok.split("=", 1)[1]
            i += 1
            continue
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "loop":
        action = (args.target or "status").strip().lower()
        loop_extras = {
            "export": bool(args.export or extras.get("export")),
            "json": bool(args.json or extras.get("json")),
            "kind": extras.get("kind") or args.kind,
            "goal": extras.get("goal") or args.goal,
            "max_turns": extras.get("max_turns")
            if "max_turns" in extras
            else args.max_turns,
            "budget_usd": extras.get("budget_usd")
            if "budget_usd" in extras
            else args.budget_usd,
            "loop_id": extras.get("loop_id") or args.loop_id,
            "reason": extras.get("reason") or args.reason,
            "iteration": extras.get("iteration")
            if "iteration" in extras
            else args.iteration,
            "agent_id": extras.get("agent_id") or args.agent_id,
        }
        # --export / --json must be stdout-clean (eval "$(…)"); no ASCII banner.
        if not (loop_extras["export"] or loop_extras["json"]):
            print_banner()
        raise SystemExit(run_loop(action, force_local=force_local, extras=loop_extras))
    ...
    if args.command == "quota":
        raise SystemExit(
            run_quota(
                force_local=force_local,
                full=bool(args.full or extras.get("full")),
            )
        )
    ...
    if args.command == "status":
        ...
        prompt_nudge = bool(getattr(args, "prompt_nudge", False) or extras.get("prompt_nudge"))
        if not (line or hook or session_start or prompt_nudge):
            print_banner()
        raise SystemExit(
            run_status(
                line=line,
