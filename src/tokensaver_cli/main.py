...
Auth (no UI required):
  login     Create Free account or sign in → saves credentials + shows API key once
  logout    Remove local credentials
  whoami    Show email, plan, verification status, API key prefix
...
Options (login):
  ...
  --key ts_…            Import an existing API key (no JWT)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "status":
        ...
        if not (line or hook):
            print_banner()
        raise SystemExit(run_status(line=line, hook=hook))
