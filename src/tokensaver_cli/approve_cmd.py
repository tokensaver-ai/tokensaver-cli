...
def _selectable_models(default_ref: str) -> list[str]:
    ...
    from tokensaver_cli.profiles import load_profiles
    ...
    _add(default_ref)
    for name in ("default", "cheap", "strong"):
        if name in profiles:
            _add(profiles[name])
...
def _prompt_pick_and_approve(default_ref: str, status: str) -> str | None:
    """List plan models, let the user pick one (or keep default). Returns chosen ref or None."""
    status_label = {
    ...
    choices = _selectable_models(default_ref)
    print(file=sys.stderr)
    print("────────────────────────────────────────", file=sys.stderr)
    print("  TokenSaver zero-trust · choose a model", file=sys.stderr)
    print("────────────────────────────────────────", file=sys.stderr)
    print(f"  Current  {default_ref}", file=sys.stderr)
    ...
    for i, mid in enumerate(choices, start=1):
        mark = " ← current" if mid == default_ref else ""
        prof = profile_by_model.get(mid)
        prof_bit = f"  [{prof}]" if prof else ""
        print(f"  {i:2}. {mid}{prof_bit}{mark}", file=sys.stderr)
    print(file=sys.stderr)
    print("  Enter = approve current · number = pick another · q = cancel", file=sys.stderr)
    try:
    ...
    if answer.isdigit():
        ...
        return None
    # Allow typing a full provider/model id
    if "/" in answer:
        return answer
    print(f"  Invalid choice {answer!r} — cancelled.", file=sys.stderr)
    return None
...
def ensure_model_approved(
    ...
    *,
    ...
    if ask and not _env_auto_approve():
        picked = _prompt_pick_and_approve(ref, status)
        if not picked:
            print(
