...
def ensure_model_approved(
    ...
    *,
    ...
    Interactive TTY + ``allow_pick``: list plan models and pick before approve.
    On Free + ``allow_pick`` (bare ``tokensaver route claude``), the list is always
    shown so the user can confirm/change — Enter selects the Free plan default.
    Explicit ``tokensaver use <id>`` / ``--model`` uses ``allow_pick=False`` and
    approves that ref directly (no catalog dump).
    ...
    # the sticky/current model is already approved — only when ``allow_pick``.
    free_pick = bool(
        allow_pick
    ...
    chosen = ref
    # Full catalog picker only when the caller asked for interactive pick
    # (bare ``route claude``). Explicit ``tokensaver use <id>`` / ``--model``
    # must approve that id without dumping hundreds of models.
    show_picker = free_pick or (
        allow_pick
        and ask
        and not _env_auto_approve()
        and status != "approved"
