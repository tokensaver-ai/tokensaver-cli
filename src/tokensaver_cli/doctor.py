...
def _probe_api_key(api_host: str, api_key: str) -> tuple[bool, str]:
    """Validate the key against GET /api/v1/sdk/me (not mere presence / MCP initialize)."""
    url = f"{api_host.rstrip('/')}/api/v1/sdk/me"
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            detail = f"{api_key[:8]}… · workspace={str(wid)[:8]}…" if wid else f"{api_key[:8]}…"
            if email:
                detail = f"{api_key[:8]}… · {email}"
            return True, detail
        ...
        if exc.code in (401, 403):
            return False, f"HTTP {exc.code} invalid/revoked key — recreate via console or `tokensaver keys` / unset stale TOKENSAVER_API_KEY"
...
def run_doctor(*, claude: bool = False, force_local: bool = False) -> int:
    ...
    checks: list[tuple[str, bool, str]] = [
        ("API key", *_probe_api_key(cfg.api_host, cfg.api_key)),
