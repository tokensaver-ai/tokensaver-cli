...
_INITIAL_WELCOME_PROMPT = (
    "Message système TokenSaver (pas une question utilisateur) : "
    "affiche brièvement le guide d'accueil (skill tokensaver-onboarding) "
    "avec invitation à ouvrir la console web (même compte que tokensaver login ; "
    "`tokensaver flows --open` ou platform.tokensaver.fr) "
    "et les actions /tokensaver-router:models, :flows, :quota, :whoami, :help. "
    "Ne demande pas de clé API. "
    "Ensuite, dès qu'un vrai message utilisateur arrive, réponds d'abord à sa question."
)
