...
When the user sets a **goal**, max turns, or asks to **govern** an iterative coding session — follow skill **`tokensaver-agentic-loops`** and/or slash `/tokensaver-router:loop`.

| Tool | Use when |
|------|----------|
| `tokensaver_loop_start` | Before iterative / goal-based work |
| `tokensaver_loop_iteration` | Milestone / next turn |
| `tokensaver_loop_precheck` | Before expensive turn (policy) |
| `tokensaver_loop_end` | Stop with reason |
| `tokensaver_loop_get` | Inspect loop |

CLI: `tokensaver loop start|tick|end|status|env`. Console: **Espace développeur** / graphe **Boucles**.

Do **not** invent a loop from wall-clock gaps alone.
