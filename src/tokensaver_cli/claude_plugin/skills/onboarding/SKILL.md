...
description: >-
  ...
  tools, models, quotas, and Flux IA. Use ONLY when the user runs
  /tokensaver-router:welcome, or explicitly asks how TokenSaver works / setup /
  help / quotas / Flux IA. Never auto-load for short greetings (salut, hello, hi,
  tu es là). Do NOT run shell or MCP for a welcome — text reply only.
---
...
**Always answer the user's message first** — with a normal short reply.

- « tu es là ? », « salut », hello → answer in **one short sentence** (e.g. « Oui, je suis là. »)
- Do **not** load this skill, do **not** dump the welcome template, do **not** run tools
- Welcome / orientation only for `/tokensaver-router:welcome` or an explicit how-to-use question

When you do show the welcome template: **text only** — no Bash, no MCP, no `tokensaver` CLI calls.
