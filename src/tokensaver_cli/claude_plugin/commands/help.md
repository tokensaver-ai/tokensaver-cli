...
| `/tokensaver-router:loop` | **BusinessLoop** start/tick/end (ACP-9 vibe coding) | `tokensaver loop …` / MCP `tokensaver_loop_*` |
...
**Agent skills**: `tokensaver-onboarding` (how-to / welcome — never replaces answering the user), `tokensaver-mcp-tools`, `tokensaver-agentic-loops` (goals → BusinessLoop), `tokensaver-ccr` (CCR markers → `tokensaver_retrieve_context` before Edit).
