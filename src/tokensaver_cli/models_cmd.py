...
def _models_for_plan(plan: str | None) -> list[str]:
    """Models shown to the user for their plan (Free = static allowlist)."""
    if is_free_plan(plan):
        return free_allowlist_models()
    return _list_api_models()
...
def run_models(*, force_local: bool = False, api_only: bool = False) -> int:
    ...
    if is_free_plan(plan):
        print("Free plan allowlist (15 hosted models)")
    else:
        print("Catalogue (OpenAI-compat /models)")
    ids = _models_for_plan(plan)
    if not ids:
