...
# Legacy local default before TokenSaver standardized on :7337.
_LEGACY_LOCAL_CONSOLE_URLS = frozenset(
    {
        "http://localhost:3000",
        "http://127.0.0.1:3000",
    }
...
def resolve_local_console_url(*, sticky: str | None = None) -> str:
    ...
    4. Sticky credentials ``console_url`` (local) — ignore legacy ``:3000`` sticky
    ...
    if (
        sticky_url
        and _is_local_host(sticky_url)
        and sticky_url not in _LEGACY_LOCAL_CONSOLE_URLS
    ):
        return sticky_url
...
def resolve_route_config(*, force_local: bool = False) -> RouteConfig:
    ...
    # Prefer keys / loop id saved by tokensaver-egress setup / claude --loop.
    ...
    if not api_key:
        raise RouteConfigError(
            ...
            "Or: tokensaver-egress setup  (uses ~/.tokensaver-egress/env)\n"
