...
# Hosted Free allowlist — must stay in sync with apps/backend/data/plan_model_access.json.
...
# Full Free hosted pack (15). Used by the zero-trust picker / ``tokensaver models``.
# Order: profiles first (cheap → default → strong), then remaining allowlist alpha.
FREE_ALLOWLIST_MODELS: tuple[str, ...] = (
...
def free_allowlist_models() -> list[str]:
    """Return the Free-plan hosted model refs (stable order)."""
    return list(FREE_ALLOWLIST_MODELS)


def is_free_allowlisted(model_id: str) -> bool:
    mid = (model_id or "").strip().lower()
    if not mid:
        return False
    return mid in {m.lower() for m in FREE_ALLOWLIST_MODELS}
