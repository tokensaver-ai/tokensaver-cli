...
def test_ensure_model_approved_already_ok(api_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert ensure_model_approved("openrouter/openai/gpt-oss-20b", interactive=False) is True
...
def test_ensure_model_approved_prompts_and_approves(
    ...
    assert (
        ensure_model_approved("openrouter/openai/gpt-oss-20b", interactive=True) is True
    )
    err = capsys.readouterr().err
    assert "zero-trust" in err.lower()
    assert "Approve this model now" not in err  # prompt goes to input(), text is on stderr above
    assert "TokenSaver zero-trust policy" in err
...
def test_ensure_model_approved_decline(
    ...
    assert (
        ensure_model_approved("openrouter/openai/gpt-oss-20b", interactive=True) is False
    )
