...
def test_ensure_model_approved_free_pick_even_when_already_approved(
    api_home: Path, monkeypatch: pytest.MonkeyPatch, capsys
) -> None:
    """route claude must still list Free models when sticky ref is already approved."""
    monkeypatch.setenv("TOKENSAVER_PLAN", "free")

    def fake_status(ref, *a, **k):
        return "approved"

    monkeypatch.setattr(
        "tokensaver_cli.approve_cmd.catalog_model_status", fake_status
    )
    monkeypatch.setattr("builtins.input", lambda _prompt="": "")
    assert (
        ensure_model_approved(
...
def test_ensure_model_approved_skips_picker_when_approved_without_allow_pick(
    api_home: Path, monkeypatch: pytest.MonkeyPatch, capsys
) -> None:
    monkeypatch.setenv("TOKENSAVER_PLAN", "free")
    monkeypatch.setattr(
    ...
    assert (
        ensure_model_approved(
            "openrouter/openai/gpt-oss-20b", interactive=True, allow_pick=False
        )
        == "openrouter/openai/gpt-oss-20b"
    )
    err = capsys.readouterr().err
    assert "already approved" in err
    assert "Free plan models" not in err


def test_ensure_model_approved_explicit_ref_skips_picker(
    api_home: Path, monkeypatch: pytest.MonkeyPatch, capsys
) -> None:
    """tokensaver use <id> must approve that id without dumping the catalog."""
    monkeypatch.setattr(
        "tokensaver_cli.approve_cmd.catalog_model_status",
        lambda *a, **k: "missing",
    )
    ...
    def fake_approve(ref, **kwargs):
        approved.append(ref)
        return {"id": "1", "ref": ref, "status": "approved"}

    monkeypatch.setattr("tokensaver_cli.approve_cmd.approve_catalog_ref", fake_approve)
    monkeypatch.setattr(
        "builtins.input",
        lambda _prompt="": (_ for _ in ()).throw(AssertionError("picker must not run")),
    )
    assert (
        ensure_model_approved(
    ...
    assert approved == ["openrouter/~anthropic/claude-sonnet-latest"]
    err = capsys.readouterr().err
    assert "approving" in err.lower()
    assert "choose a model" not in err.lower()
