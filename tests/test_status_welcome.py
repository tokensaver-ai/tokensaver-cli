...
from tokensaver_cli.status import (
    format_session_start_json,
    format_welcome_context,
    format_welcome_initial_user_message,
)
...
def test_welcome_context_includes_route_facts(ts_home: Path) -> None:
    ...
    with patch("tokensaver_cli.status.fetch_recent_flow_id", return_value=None):
        ...
        init_msg = format_welcome_initial_user_message(cfg)
    assert "gpt-oss-20b" in text
    assert "TokenSaver" in text
    assert "WELCOME BLOCK" in text
    assert "Bienvenue sur TokenSaver" in text
    assert "Flux IA" in text
    assert "ws-1" in text or "Flux IA" in text
    assert "Bienvenue" in init_msg
    assert "Bash" in init_msg or "MCP" in init_msg
...
def test_welcome_json_is_valid_session_start_hook(ts_home: Path) -> None:
    ...
    assert "Bienvenue" in data["hookSpecificOutput"]["initialUserMessage"]
    assert data["hookSpecificOutput"].get("reloadSkills") is not True
    assert "WELCOME BLOCK" in data["hookSpecificOutput"]["additionalContext"]
    assert "Bienvenue sur TokenSaver" in data["hookSpecificOutput"]["additionalContext"]
