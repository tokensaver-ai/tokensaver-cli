...
def test_route_claude_writes_settings_and_mcp(route_home: Path, cfg: RouteConfig, tmp_path: Path) -> None:
    ...
    assert "tokensaver status --line" in settings["statusLine"]["command"]
    assert settings["hooks"]["SessionStart"]
    hook_cmds = json.dumps(settings["hooks"]["SessionStart"])
...
def test_route_claude_no_unknown_model_env_for_anthropic_native(
    ...
    try:
        os.chdir(project)
        with patch("tokensaver_cli.approve_cmd.ensure_model_approved", return_value=True):
            assert (
                route_claude(
                    cfg,
                    launch=False,
...
def test_route_claude_launch_aborts_when_model_not_approved(
    ...
    try:
        os.chdir(project)
        with patch("tokensaver_cli.approve_cmd.ensure_model_approved", return_value=False):
            assert (
                route_claude(
                    cfg,
                    launch=True,
