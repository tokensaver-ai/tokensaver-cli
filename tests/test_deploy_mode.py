...
def test_resolve_route_config_local_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "tokensaver_cli.config.discover_local_console_url", lambda: None
    )
    monkeypatch.setattr(
        "tokensaver_cli.credentials.load_credentials", lambda: None
    )
    ...
    assert resolved.console_url == "http://localhost:7337"


def test_resolve_route_config_local_console_from_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("TOKENSAVER_API_KEY", "ts_x")
    monkeypatch.setenv("TOKENSAVER_MODE", "local")
    monkeypatch.delenv("TOKENSAVER_CONSOLE_URL", raising=False)
    monkeypatch.setenv("CONSOLE_BASE_URL", "http://localhost:7337")
    monkeypatch.setattr(
        "tokensaver_cli.config.discover_local_console_url", lambda: None
    )
...
def test_resolve_route_config_local_console_tokensaver_env_wins(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("TOKENSAVER_API_KEY", "ts_x")
    monkeypatch.setenv("TOKENSAVER_MODE", "local")
    monkeypatch.setenv("TOKENSAVER_CONSOLE_URL", "http://localhost:7337")
    monkeypatch.setenv("CONSOLE_BASE_URL", "http://localhost:3000")
    monkeypatch.setattr(
        "tokensaver_cli.config.discover_local_console_url", lambda: "http://localhost:9999"
    )
...
def test_resolve_local_console_ignores_legacy_sticky_3000(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from tokensaver_cli.config import resolve_local_console_url
    ...
    assert (
        resolve_local_console_url(sticky="http://localhost:3000")
        == "http://localhost:7337"
    )
    assert (
        resolve_local_console_url(sticky="http://localhost:9000")
        == "http://localhost:9000"
    )


def test_discover_local_console_url_from_console_port(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from tokensaver_cli.config import discover_local_console_url

    console_dir = tmp_path / "apps" / "console"
    console_dir.mkdir(parents=True)
    (console_dir / ".env.local").write_text("PORT=7337\n", encoding="utf-8")
    (tmp_path / "packages" / "cli").mkdir(parents=True)
    monkeypatch.chdir(tmp_path)

    assert discover_local_console_url() == "http://localhost:7337"
...
def test_login_ignores_localhost_env_without_local_flag(
    ...
    monkeypatch.setenv("TOKENSAVER_CONSOLE_URL", "http://localhost:7337")
    assert _console_url(force_local=True) == "http://localhost:7337"
