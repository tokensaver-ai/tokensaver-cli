...
def test_save_load_clear_credentials(ts_home: Path) -> None:
    ...
    assert resolve_api_key() == "ts_abc123456789"
...
def test_login_signup_flow(ts_home: Path) -> None:
    ...
    creds = load_credentials()
    assert creds is not None
    assert creds.api_key == "ts_signup_key_plainxx"
    assert creds.access_token == "jwt-signup"
    assert creds.organisation_id == "org-1"


def test_login_signin_creates_cli_key(ts_home: Path) -> None:
    calls: list[str] = []
    ...
    def fake_request(method, url, **kwargs):
        ...
        if url.endswith("/api-keys") and method.upper() == "POST":
            return {"api_key": "ts_cli_created_keyxxxx", "id": "key-9", "key_prefix": "ts_cli_crea"}
        if url.endswith("/users/me"):
            return {
    ...
    creds = load_credentials()
    assert creds is not None
    assert creds.api_key == "ts_cli_created_keyxxxx"
    assert creds.key_id == "key-9"
    assert any("/api-keys" in c for c in calls)
...
def test_keys_list_create_revoke(ts_home: Path) -> None:
    ...
    with patch("tokensaver_cli.keys_cmd.request_json", side_effect=fake_request):
        ...
        creds = load_credentials()
        assert creds is not None
        assert creds.api_key == "ts_new_secret_zzzz"
